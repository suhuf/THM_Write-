...
To view alerts and alert signatures within PCAP files, we need to use **Brim.** We can select **Brim** and upload the PCAP file; then view the logs in Brim's **GUI**.
...
We can do this by asking for all logs where the event type is "alert" and then using a pipe (|) to also request that it only have logs that the **alert.category** (Alert Category field) matches **"A Network Trojan was Detected"**
...
Here we can use the same filter as before, except changing the category to: **Potential Corporate Privacy Violation.**
...
\*Note that it was requested that our answer be "Defanged" and I have provided the answer in _Defanged_ form. Defanging is when a malicious link or IP address is provided in a safer format that does not lead to detonation (exaggerative
...
We need to find the download and its detailed information. To do this, we right-click on the packet, hover over **Follow**, and then select **TCP Stream:**
...
We can now see the details we needed in the **TCP Stream:**

<img width="639" height="623" alt="image" src="https://github.com/user-attachments/assets/80cbd46b-d7f5-451c-9373-c6e8e7a7034b" />

In the **Host** area, we can see the **Host** domain that is associated with the URL the user downloaded from:

**awh93dhkylps5ulnq-be\[.\]com**

In the **GET** section, we can also see the exact page they requested from the domain, meaning the last part of the URL:
...
Now we need to get the hash of the file. We can open a terminal session and move to the respective directory. In my case, it is in Downloads, and I have renamed the file to "**dragon.cab**" so it is easier to interact with without all of the special characters.

We now use this command: **md5sum** and copy the hash we are given:

<img width="725" height="486" alt="image" src="https://github.com/user-attachments/assets/dde110e0-685d-47a3-90ae-68d3b272a028" />

And we can now search VirusTotal and see what results we get when we put the **MD5Hash** in the search bar:
...
Again, we select the packet, right-click, and select Follow **TCP stream:**
...
For this, we should move back to **Brim**. Let's reset the work we did in **Brim** and return to the base **.pcap** file in **Brim**.

Let's now set a filter that does these 3 things:
...
then only includes unique results.
...
For this section, we should recognize that VirusTotal results are not consistent over time; VirusTotal is a corroboration of analysis from multiple security vendors, which is also updated over time to include more security vendors of varying values.

As a result, over time it is more likely that a domain that was considered safe at one time will later be judged as malicious, as new vendors can have less rigorous requirements on what they label as malicious.

So we need to factor this in when looking at the results for this lab that is at least 3-4 years old.
...
We are able to see here that we are supposed to only be factoring logs that have triggered the **"Misc Activity"** **Alert Category**. We can implement a filter that factors this and see what overlaps with the domains we have identified in **VirusTotal:**
...
We can check this IP **185\[.\]118\[.\]164\[.\]8** in **VirusTotal** and check the associated domains in the Relations tab:
...
Let's check the info on VirusTotal for these domains:
...
For this, we can filter our logs according to the alert category **"Not Suspicious Traffic"** and cut for either the source or destination IP depending on what the traffic is. Let's cut according to source IP (src_ip):

We are going to use this command: **alert.category == "Not Suspicious Traffic"** to get the non-suspicious traffic, **cut src_ip** to isolate the source IP, **sort** to organize in preparation for **uniq**, and then add **uniq** after a pipe. Altogether, it should look like this: **alert.category == "Not Suspicious Traffic" | cut src_ip | sort | uniq**
...
Let's put **64\[.\]255\[.\]65\[.\]166** in VirusTotal and find domains that match what we are looking for:
...
Our answer, defanged and in alphabetical order, should be:  
<br/>**safebanktest\[.\]top, tocsicambar\[.\]xyz, ulcertification\[.\]xyz**
...
We can put this IP in VirusTotal and perform the same actions we did earlier:
