...
In the log, we can see that this user is using the **whoami** command to see what user they are currently logged in as an enumerating which groups they are apart of which ties into their permissions.
...
This may suggest that this account was created in an attempt to create **persistence** on the system via a new user as a backdoor they can return to in the future. This is a tactic mention by **Mitre**
...
2nd. We need to isolate them according to using a System process/LOLBIN.
...
We have now narrowed our logs from 14k to around 4.5k. However, this is still too many logs. We should move to our next attribute we should filter accoriding too:

Our 3rd attribute is the to filter downloads. We can find some downloads if we comb the logs for common domain names like .com, lets add the string ".com" so that our filter only contains logs with this string somewhere in it:
...
From the command line area, we can see that the user **haroon** is using a system process (LOLBin) to download contents form controlc.com (which is a paste bin domain) that is being saved as an .exe called **benign**

We can also confirm this is a LOLBin as the binary is present in **Windows\System32**

This LOLBin also is specifically commonly used by threat actors to download payload stages:

**Mitre** has outlined this speific tactic here as **T1105** (Tactic): 

https://attack.mitre.org/techniques/T1105/

**Mitre** description of **certutil** uses and other information: https://attack.mitre.org/software/S0160/ 
...
We were able to highlight this earlier, it is **controlc.com** which is a pastebin site
...
We can visit the site our self and check this since this is a lab environment:
...
We covered this earlier, we identified **https://controlc.com/e4d11035** as the malicious **URL.**
